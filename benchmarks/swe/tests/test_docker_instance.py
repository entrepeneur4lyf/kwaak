...
def test_docker_instance_run(mock_swe_instance, temp_results_dir):
    ...
    try:
        ...
        cache_mount = next(
            m for m in mounts if m["Destination"] == "/root/.cache/kwaak"
        )
        ...
        log_mount = next(
            m for m in mounts if m["Destination"] == "/root/.cache/kwaak/logs"
        )
        ...
        # Verify platform
        assert (
            instance.container.attrs["Platform"] == "linux"
        )  # Docker API returns just 'linux'
